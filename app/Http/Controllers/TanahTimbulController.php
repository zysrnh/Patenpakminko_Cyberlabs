...
class TanahTimbulController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->isPelakuUsaha()) {
            $applications = TanahTimbulApplication::where('user_id', $user->id)
    ...
    {
        if (!Auth::check()) {
            return redirect()->route('ptp.create', ['layanan' => 'tanah-timbul', 'new' => 1])->with('info', 'Silakan isi formulir Permohonan PTP terlebih dahulu.');
        }
        ...
        return view('tanah-timbul.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'nib'                       => ($request->hasFile('nib') || $request->filled('temp_nib')) ? 'nullable' : 'required',
        ...
        foreach ($filesToStore as $fileKey) {
            if ($request->hasFile($fileKey)) {
                ...
                $data[$fileKey] = $file->storeAs('tanah-timbul_docs', $fileName, 'public');
    ...
    public function show($id)
    {
        ...
        return view('tanah-timbul.show', compact('application'));
    }
    ...
    public function verify(Request $request, $id)
    {
        ...
        if ($user->isBpn() && $application->status === 'menunggu_bpn' && $step === 'bpn_cek_lokasi') {
            ...
            return redirect()->route('tanah-timbul.show', $id)->with('success', 'Jadwal Cek Lokasi Lapangan berhasil disimpan & di-blast ke WhatsApp pemohon.');
        }
        ...
        if ($user->isBpn() && $application->status === 'menunggu_bpn' && $step === 'bpn_rapat') {
            ...
            return redirect()->route('tanah-timbul.show', $id)->with('success', 'Jadwal Rapat Koordinasi berhasil disimpan & di-blast ke WhatsApp pemohon.');
        }
    ...
    public function destroy($id)
    {
        ...
        $app = \App\Models\TanahTimbulApplication::findOrFail($id);
        ...
        $app->delete();
        return redirect()->route('tanah-timbul.index')->with('success', "Permohonan {$appNo} berhasil dihapus dari antrean.");
    }
}
