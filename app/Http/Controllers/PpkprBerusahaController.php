...
class PpkprBerusahaController extends Controller
{
    ...
    {
        if (!Auth::check()) {
            return redirect()->route('ptp.create', ['layanan' => 'berusaha', 'new' => 1])->with('info', 'Silakan isi formulir Permohonan PTP terlebih dahulu.');
        }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'peta_lokasi.max'                   => 'Ukuran file peta lokasi maksimal 100MB.',
            'fc_ktp.required'                   => 'Fotokopi KTP wajib diunggah.',
            'fc_ktp.mimes'                      => 'FC KTP harus berformat PDF, JPG, atau PNG.',
    ...
    public function verify(Request $request, $id)
    {
        ...
        if ($user->isBpn()) {
            ...
            // BPN Langkah 1: Verifikasi Dokumen Persyaratan & Upload SPS
            if ($step === 'bpn_berkas') {
                $request->validate([
                ...
                if ($action === 'approve') {
                    if (!$request->hasFile('sps_document') && !$application->bpn_sps_document) {
                        return redirect()->back()->withErrors(['sps_document' => 'Dokumen SPS wajib diunggah saat menyetujui berkas (Lengkap).']);
                    }
                    ...
                    $msg = 'Berkas disetujui, Dokumen SPS berhasil diunggah & tautan di-blast ke WhatsApp pemohon.';
                } else {
