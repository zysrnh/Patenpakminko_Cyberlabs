        ...
        if(panel) {
            ...
            if(index === 0) {
                ...
                prevBtn.onclick = function(e) {
                    e.preventDefault();
                    const targetStep = timelineSteps[index - 1];
                    const tMatch = targetStep.getAttribute('onclick').match(/showBpnPanel\\(['"]?([^'"]+)['"]?\\)/);
                    if(tMatch) {
                        const tPanel = document.getElementById('bpn-panel-' + tMatch[1]);
                        document.querySelectorAll('.bpn-panel-step').forEach(p => p.style.display = 'none');
                        if(tPanel) { tPanel.style.display = 'block'; tPanel.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
                        document.querySelectorAll('.timeline-step').forEach(ts => ts.classList.remove('viewing-step'));
