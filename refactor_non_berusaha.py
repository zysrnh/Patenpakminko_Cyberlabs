...
with open('resources/views/non-berusaha/show.blade.php', 'r', encoding='utf-8') as f:
    content = f.read()
...
content = content.replace(
    "@php $isStep2Active = ($application->bpn_berkas_status === 'diterima' && $application->bpn_pembayaran_status === 'menunggu'); @endphp",
...
content = content.replace(
    "@php $isStep4Active = ($application->bpn_cek_lokasi_dt && $cekLokasiLewat); @endphp",
...
content = content.replace(
    "@php $isStep5Active = ($application->bpn_rapat_dt && $rapatLewat && !$application->bpn_pertek_document); @endphp",
...
content = re.sub(r'<div class="timeline-step \{\{ \$step6Status \}\}">\s*<span class="timeline-dot"></span>\s*<div class="timeline-title">5\. Penilaian PKKPR \(Dinas PUTR\)</div>', 
...
content = re.sub(r'<div class="timeline-step \{\{ \$step7Status \}\}">\s*<span class="timeline-dot"></span>\s*<div class="timeline-title">6\. Penerbitan PKKPR \(Dinas PMPTSP\)</div>', 
...
with open('resources/views/non-berusaha/show.blade.php', 'w', encoding='utf-8') as f:
    f.write(content)

print("Done non-berusaha")
