...
<div class="panel">
    @if($applications->isEmpty())
        <div class="empty-state">
            ...
            @if(Auth::user()->isPelakuUsaha())
                <a href="{{ route('ptp.create', ['layanan' => 'tanah-timbul']) }}" class="btn btn-primary">Ajukan Sekarang</a>
            @endif
        </div>
    @else
                <!-- Filter Controls -->
                    ...
                    @foreach($applications as $app)
                        <tr>
                            ...
                            <td style="display:flex; gap:6px; align-items:center; flex-wrap:wrap;">
                                <a href="{{ route('tanah-timbul.show', $app->id) }}" class="btn btn-sm btn-secondary">
                                    Detail
                                    <svg viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                                </a>
                                @if(Auth::user()->isDpn())
                                <form action="{{ route('tanah-timbul.destroy', $app->id) }}" method="POST" onsubmit="return confirm('Hapus permanen permohonan {{ $app->application_number }}? Data tidak bisa dikembalikan!')">
                                    @csrf @method('DELETE')
