        ...
        <div class="container">
            ...
            @if($application->bpn_pertek_document)
                @php
                    $review = \App\Models\Review::where('user_id', Auth::id())
                        ->where('module_type', 'tanah-timbul')
                    ...
                    <div class="verify-card" style="border-color: #CBD5E0; background: #F8FAFC; margin-bottom: 24px; padding: 24px;">
                        ...
                        @else
                            ...
                            <form action="{{ route('review.store') }}" method="POST">
                                @csrf
                                <input type="hidden" name="module_type" value="tanah-timbul">
            ...
            @php
                ...
                if ($user->isBpn() && $application->status === 'menunggu_bpn') {
                    $canVerify = true;
                    $verifierRoleLabel = 'Petugas BPN (Verifikasi tanah-timbul)';
                }
                ...
                <div class="verify-card">
                    ...
                    @if($user->isBpn() && $application->status === 'menunggu_bpn')
                        ...
                        @elseif($application->bpn_berkas_status === 'diterima')
                            ...
                            {{-- ===== Form Jadwal Cek Lokasi (Selalu bisa diisi/diedit) ===== --}}
                            ...
                            @if($application->bpn_cek_lokasi_dt)
                                <hr style="border:none;border-top:1px solid var(--clr-line);margin:24px 0;">
                                <form action="{{ route('tanah-timbul.verify', $application->id) }}" method="POST" style="margin-bottom: 24px;">
                                    @csrf
                    ...
                    <div class="card">
                        ...
                        <ul class="detail-list">
                                ...
                                <span class="detail-val" style="display: flex; align-items: center; gap: 8px;">
                                    ...
                                    @if(Auth::user()->isBpn())
                                        ...
                                        <div style="display: inline-flex; gap: 4px; margin-left: 8px;">
                                            <form action="{{ route('application.rollback', ['tanah-timbul', $application->id]) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan ke tahap sebelumnya?')">
                                                @csrf
                                            ...
                                            <form action="{{ route('application.forward', ['tanah-timbul', $application->id]) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('Bypass ke tahap selanjutnya?')">
                                                @csrf
                            ...
                            <li class="detail-item" style="display: flex; flex-direction: column; gap: 8px;">
                                ...
                                @php
                                    $jenisPermohonan = 'tanah-timbul';
                                    if ($application->ptp_data) {
                                        $ptp = json_decode($application->ptp_data, true);
                                        $jenisPermohonan = $ptp['jenis_permohonan'] ?? 'tanah-timbul';
                                    }
                                @endphp
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; padding-bottom: 12px; border-bottom: 1px dashed var(--clr-line);">
                                    ...
                                    <a href="{{ route('tanah-timbul.ptp_pdf', $application->id) }}" target="_blank" class="btn-doc">
                            ...
                            <div class="timeline-step {{ $doneStepStatus }}">
                                ...
                                <div class="timeline-desc">
                                    ...
                                    @elseif($application->status === 'disetujui')
                                        Dokumen Pertek Pertanahan / Surat Rekomendasi Tanah Timbul siap diunduh.
                                    @else
