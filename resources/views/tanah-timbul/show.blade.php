                ...
                <div class="nav-menu">
                    ...
                    <div class="user-nav" style="margin-left: 12px; padding-left: 12px; border-left: 1.5px solid var(--clr-line);">
                        @if(Auth::user()->profile_photo)
                            <img src="{{ route('file.view', ['path' => Auth::user()->profile_photo]) }}" alt="Foto Profil" class="header-avatar">
                        @else
    ...
    <main>
        <div class="container">
            ...
            <div class="page-header">
                <div>
                    <a href="{{ route('tanah-timbul.index') }}" class="back-btn">
                    ...
                    <div style="display: flex; align-items: center; gap: 14px;">
                        ...
                        <div>
                            <div style="font-size: 14.5px; font-weight: 800; color: #065F46; margin-bottom: 2px;">
                                Dokumen Pertek Pertanahan Tanah Timbul (BPN) Telah Terbit!
                            </div>
            ...
            @if($application->bpn_pertek_document)
                @php
                    $review = \App\Models\Review::where('user_id', Auth::id())
                        ->where('module_type', 'tanah_timbul')
                    ...
                    <div class="verify-card" style="border-color: #CBD5E0; background: #F8FAFC; margin-bottom: 24px; padding: 24px; border-radius: 12px; border: 1.5px solid var(--clr-line);">
                        ...
                        @else
                            ...
                            <form action="{{ route('review.store') }}" method="POST">
                                @csrf
                                <input type="hidden" name="module_type" value="tanah_timbul">
            ...
            @if(Auth::user()->isDpn() && !($application->status === 'menunggu_bpn' && ($application->bpn_berkas_status === 'menunggu' || !$application->bpn_berkas_status)))
                <div class="verify-card" style="border: 1px solid #FBD38D; background: #FFFAF0; margin-bottom: 24px; padding: 16px; border-radius: 8px;">
                    ...
                    <form action="{{ route('application.rollback', ['type' => 'tanah-timbul', 'id' => $application->id]) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mengembalikan status permohonan ini ke tahap sebelumnya?');">
                        @csrf
            ...
            @if(false)
                <div class="verify-card" style="border: 1px solid #B2F5EA; background: #F0FFF4; margin-bottom: 24px; padding: 16px; border-radius: 8px;">
                    ...
                    <form action="{{ route('application.forward', ['type' => 'tanah-timbul', 'id' => $application->id]) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin memajukan status permohonan ini?');">
                        @csrf
            ...
            @php
                ...
                if ($user->isBpn() && $application->status === 'menunggu_bpn') {
                    $canVerify = true;
                    $verifierRoleLabel = 'Verifikator (Verifikasi Kepemilikan Tanah)';
                        ...
                        <div id="Kantor Pertanahan-panel-1" class="Kantor Pertanahan-panel-step" style="display: {{ in_array($application->bpn_berkas_status, ['menunggu', 'diterima', 'tidak_sesuai', 'ditolak']) ? 'block' : 'none' }};">
                            ...
                            <fieldset {{ $isStep1Active ? '' : 'disabled' }}>
                                <form action="{{ route('tanah-timbul.verify', $application->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <script>
                                        function toggleBpnSpsField(action) {
                                    ...
                                    <div class="form-group-v" style="margin-bottom: 20px;">
                                        ...
                                        <div style="display: flex; gap: 20px;">
                                            <label style="display:flex;align-items:center;gap:6px;font-size:13.5px;font-weight:600;cursor:pointer;">
                                                <input type="radio" name="action" value="approve" required {{ $application->bpn_berkas_status === 'diterima' ? 'checked' : ($application->bpn_berkas_status === 'ditolak' || $application->bpn_berkas_status === 'tidak_sesuai' ? '' : 'checked') }} onchange="toggleBpnSpsField('approve')" style="width:16px;height:16px;accent-color:var(--clr-blue);"> Disetujui / Lengkap
                                            </label>
                                            <label style="display:flex;align-items:center;gap:6px;font-size:13.5px;font-weight:600;color:#E53E3E;cursor:pointer;">
                                                <input type="radio" name="action" value="reject" required {{ $application->bpn_berkas_status === 'ditolak' || $application->bpn_berkas_status === 'tidak_sesuai' ? 'checked' : '' }} onchange="toggleBpnSpsField('reject')" style="width:16px;height:16px;accent-color:var(--clr-blue);"> Tidak Lengkap
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <div id="sps-upload-container" style="display: {{ in_array($application->bpn_berkas_status, ['diterima', 'menunggu']) && $application->bpn_berkas_status !== 'tidak_sesuai' && $application->bpn_berkas_status !== 'ditolak' ? 'block' : 'none' }}; margin-bottom: 20px;">
                                        ...
                                        <input type="file" name="sps_document" id="sps_document" class="form-control-v" accept=".pdf,.jpg,.jpeg,.png" {{ in_array($application->bpn_berkas_status, ['diterima', 'menunggu']) && $application->bpn_berkas_status !== 'tidak_sesuai' && $application->bpn_berkas_status !== 'ditolak' ? 'required' : '' }}>
                                        <div style="font-size: 11.5px; color: var(--clr-muted); margin-top: 5px;">Maksimal 100MB. Wajib diisi jika Berkas Lengkap (Disetujui). File SPS ini akan dikirimkan otomatis ke WhatsApp Pemohon.</div>
                                    </div>
                                    ...
                                    <div id="revisi-berkas-container" style="display:none; margin-bottom: 12px; background: #fff5f5; padding: 12px; border: 1px solid #fed7d7; border-radius: 4px;">
                                        ...
                                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 6px; font-size: 12px;">
                                            ...
                                            <label><input type="checkbox" class="cb-revisi" value="Surat Keterangan Tanah Timbul"> Surat Keterangan Tanah Timbul</label>
                                            <label><input type="checkbox" class="cb-revisi" value="Rencana Penggunaan Tanah"> Rencana Penggunaan Tanah</label>
                                            <label><input type="checkbox" class="cb-revisi" value="Sporadik"> Surat Pernyataan Fisik (Sporadik)</label>
                                    ...
                                    @if($isStep1Active)
                                        ...
                                        <button type="submit" class="btn-submit-v">Kirim Hasil Verifikasi Berkas & Notifikasi WA</button>
                                    @else
                                ...
                                <form action="{{ route('tanah-timbul.verify', $application->id) }}" method="POST">
                                    ...
                                    @if($isStep2Active)
                                        ...
                                        <button type="submit" class="btn-submit-v" style="font-size:13px;padding:10px 20px;">
                                            💰 Konfirmasi Pembayaran & Notifikasi WA Akun Pemohon
                                        </button>
                                    @else
                            ...
                            @if(Auth::user()->isBpn() && $application->bpn_pembayaran_status === 'sudah_bayar')
                                <form action="{{ route('tanah-timbul.verify', $application->id) }}" method="POST" style="margin-top: 16px;">
                                    ...
                                    <button type="submit" class="btn-submit-v" style="background: var(--clr-blue); width: 100%; justify-content: center;">
                                        ...
                                        Kirim Ulang Akun & Kredensial (WhatsApp)
                                    </button>
                                </form>
                            @endif
                        </div>
                                ...
                                <form action="{{ route('tanah-timbul.verify', $application->id) }}" method="POST">
                                    ...
                                    <div class="form-group-v">
                                        ...
                                        <input type="text" name="bpn_cek_lokasi_cp" class="form-control-v"
                                            ...
                                            placeholder="cth: 08511234567 (Budi - Petugas Instansi)" style="background:white;" required>
                                    </div>
                                    @if($isStep3Active)
                                        <div class="form-group-v" style="margin-bottom: 12px; border-top: 1px dashed var(--clr-line); padding-top: 12px; margin-top: 12px;">
                                            <label style="font-size: 11.5px; color: var(--clr-muted); font-weight: 600;">✎ Edit Pesan WA (Opsional):</label>
                                        ...
                                        <button type="submit" class="btn-submit-v" style="font-size:13px;padding:10px 20px;">
                                            {{ $application->bpn_cek_lokasi_dt ? '🔄 Ubah Jadwal Peninjauan Lapangan & Kirim WA' : '📍 Simpan Jadwal Peninjauan Lapangan & Kirim WA' }}
                                        </button>
                                    @else
                            ...
                            <fieldset {{ $isStep4Active ? '' : 'disabled' }}>
                                <form action="{{ route('tanah-timbul.verify', $application->id) }}" method="POST">
                                    ...
                                    <div style="background:#EBF8FF;border:1px solid #90CDF4;padding:12px 16px;border-radius:8px;font-size:13px;color:#2B6CB0;margin-bottom:16px;">
                                        ...
                                        @else
                                            — Peninjauan lapangan terdaftar. Tentukan waktu rapat koordinasi.
                                        @endif
                                    </div>
                                    <div class="form-group-v" style="margin-bottom:12px;">
...
<div id="Kantor Pertanahan-panel-5" class="Kantor Pertanahan-panel-step" style="display: {{ $application->bpn_rapat_dt && $rapatLewat && !$application->bpn_pertek_document ? 'block' : 'none' }};">
                            ...
                            <fieldset {{ $isStep5Active ? '' : 'disabled' }}>
                                <form action="{{ route('tanah-timbul.verify', $application->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <input type="hidden" name="wa_type" value="pertek_terbit">
                                    ...
                                    <div class="form-group-v">
                                        <label for="action">Keputusan Akhir Kantor Pertanahan:</label>
                                        <select name="action" id="action" class="form-select-v" required>
                                            <option value="approve" {{ $application->bpn_pertek_document || $application->status === 'disetujui' || $application->status === 'menunggu_dinas_pu' || $application->status === 'menunggu_satu_pintu' ? 'selected' : 'selected' }}>Disetujui</option>
                                    ...
                                    @endif
                                    <div class="form-group-v">
                                        <label class="form-label" style="font-weight:700;color:#744210;">Catatan / Rekomendasi Teknis <span style="color:red;">*</span></label>
                                    ...
                                    @if($isStep5Active)
                                        ...
                                        <button type="submit" class="btn-submit-v" style="background:#79A73A;">📄 Terbitkan Pertek & Notifikasi WA Pemohon</button>
                                    @else
                        ...
                        <div id="Kantor Pertanahan-panel-satu-pintu" class="Kantor Pertanahan-panel-step" style="display: {{ in_array($application->status, ['menunggu_satu_pintu', 'disetujui']) ? 'block' : 'none' }};">
                            ...
                            <fieldset {{ $isSpActive ? '' : 'disabled' }}>
                                <form action="{{ route('tanah-timbul.verify', $application->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group-v">
                                        ...
                                        <select name="action" id="action_sp" class="form-select-v" required onchange="toggleSatuPintuFields(this.value === 'approve')">
                                    ...
                                    <div id="satuPintuFieldsWrapper">
                                        ...
                                        <div class="form-group-v" style="margin-bottom:12px;">
                                            ...
                                            @else
                                                ...
                                                <span style="font-size:11.5px;color:#744210;margin-top:4px;display:block;">Format PDF, maks. 100MB. Dokumen ini dapat diunduh oleh pemohon.</span>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="form-group-v">
                            ...
                            @if(Auth::user()->isSatuPintu() && $application->approval_document)
                                <form action="{{ route('tanah-timbul.verify', $application->id) }}" method="POST" style="margin-top: 16px;">
                                    @csrf
                                    <input type="hidden" name="step" value="resend_wa">
                                    <input type="hidden" name="wa_type" value="pkkpr_terbit">
                                    <div class="form-group-v" style="margin-bottom: 12px; text-align: left;">
                                        <label style="font-size: 11px; color: var(--clr-muted);">Edit Pesan WA (Opsional):</label>
                                        <textarea name="custom_wa_message" class="form-control-v" rows="2" placeholder="Tuliskan pesan khusus jika ingin mengganti template bawaan..."></textarea>
                                    </div>
                        ...
                        <ul class="detail-list">
                            ...
                            @if($application->bpn_pertek_document)
                                <li class="detail-item" style="padding: 12px 0;">
                                    <span class="detail-label" style="font-weight: 800; color: #065F46;">Dokumen Pertek Pertanahan Tanah Timbul (BPN)</span>
                                    <span class="detail-val">
                                        <a href="{{ route('file.view', ['path' => $application->bpn_pertek_document]) }}" target="_blank" style="display: inline-flex; align-items: center; gap: 8px; background: linear-gradient(135deg, #10B981 0%, #059669 100%); color: #ffffff; padding: 10px 18px; border-radius: 8px; text-decoration: none; font-size: 13px; font-weight: 800; box-shadow: 0 4px 12px rgba(5, 150, 105, 0.25); transition: all 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='none'">
                                            <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                            📄 Unduh Pertek Pertanahan Tanah Timbul (BPN)
                                        </a>
                                    </span>
                                </li>
                            @endif
                            ...
                            @if($application->approval_document)
                                <li class="detail-item" style="padding: 12px 0;">
                                    <span class="detail-label" style="font-weight: 800; color: #B45309;">Dokumen PKKPR Tanah Timbul (Final)</span>
                                    <span class="detail-val">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    textareas.forEach(ta => {
        ...
        if (waTypeValue) {
            ...
            if(window.location.pathname.includes('TANAH-TIMBUL')) type = 'TANAH-TIMBUL';
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (approvalUrl && !localStorage.getItem('downloaded_doc_approval_' + appId)) {
                ...
                activeTitle = 'Dokumen PKKPR Tanah Timbul Telah Terbit!';
                activeDesc = 'Selamat! Dokumen resmi PKKPR Tanah Timbul milik Anda telah diterbitkan secara lengkap.';
            } else if (pertekUrl && !localStorage.getItem('downloaded_doc_pertek_' + appId)) {
                activeDocKey = 'pertek_' + appId;
                activeUrl = pertekUrl;
                activeTitle = 'Dokumen Pertek Pertanahan Tanah Timbul Telah Terbit!';
