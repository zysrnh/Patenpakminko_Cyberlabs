...
@section('title', 'Pertimbangan Teknis Pertanahan PKKPR Berusaha — PATEN PAK MIKO')
@section('page-title', 'Pertimbangan Teknis Pertanahan PKKPR Berusaha')
...
<div style="background: #ffffff; border: 1px solid #E2E8F0; border-radius: 6px; padding: 18px 24px; margin-bottom: 20px; box-shadow: 0 2px 6px rgba(0,38,66,0.02); display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-wrap: wrap;">
    <div>
        <div style="font-size: 12px; color: #64748B; margin-bottom: 4px; display: flex; align-items: center; gap: 6px;">
            ...
            <span>›</span>
            <span style="color: #64748B;">Pertimbangan Teknis Pertanahan PKKPR Berusaha</span>
        </div>
        ...
        <p style="font-size: 12.5px; color: #64748B; margin: 4px 0 0;">Perizinan bisnis terpadu — skala mikro hingga besar.</p>
    </div>
    @if(Auth::user()->isPelakuUsaha())
        <a href="{{ route('ptp.create', ['layanan' => 'berusaha', 'new' => 1]) }}" class="btn btn-primary" style="border-radius: 4px; padding: 8px 16px; font-weight: 700; font-size: 13px;">
...
<div class="panel" style="border-radius: 6px; border: 1px solid #E2E8F0; box-shadow: 0 2px 6px rgba(0,38,66,0.02); overflow: hidden; background: #ffffff;">
    @if($applications->isEmpty())
        <div class="empty-state" style="padding: 40px 20px; text-align: center;">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="1.5"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            <h3 style="font-size: 15px; font-weight: 700; color: #0F172A; margin: 12px 0 4px;">Belum Ada Permohonan</h3>
            <p style="font-size: 13px; color: #64748B; margin-bottom: 16px;">
                @if(Auth::user()->isPelakuUsaha())
                    Anda belum mengajukan permohonan Pertimbangan Teknis Pertanahan PKKPR Berusaha.
                @else
                    Tidak ada antrean berkas yang menunggu verifikasi.
                @endif
            </p>
            @if(Auth::user()->isPelakuUsaha())
                <a href="{{ route('ptp.create', ['layanan' => 'berusaha', 'new' => 1]) }}" class="btn btn-primary" style="border-radius: 4px;">Ajukan Sekarang</a>
            @endif
        </div>
    @else
        <!-- Filter Controls -->
        ...
        @if(Auth::user()->isDpn())
        <form id="bulkForm" action="{{ route('berusaha.bulk-destroy') }}" method="POST">
            @csrf
                ...
                <tbody>
                    @foreach($applications as $app)
                        <tr style="border-bottom: 1px solid #F1F5F9;">
                            ...
                            @endif
                            <td style="padding: 12px 14px;"><span style="font-family:'DM Mono',monospace;font-size:12px;font-weight:700;color:#003B64;">{{ $app->application_number }}</span></td>
                            ...
                            <td style="padding: 12px 14px; white-space: nowrap; text-align: center;">
                                <div style="display:inline-flex; gap:6px; align-items:center;">
                                    <a href="{{ route('berusaha.show', $app->id) }}" class="btn btn-sm btn-secondary" style="border-radius: 4px; font-size: 12px; font-weight: 700; padding: 5px 10px; display: inline-flex; align-items: center; gap: 4px;">
                                        Detail
                                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                                    </a>
                                    @if($app->ptp_data)
                                     <a href="{{ route('berusaha.ptp_pdf', $app->id) }}" target="_blank" class="btn btn-sm" style="border-radius: 4px; font-size: 12px; font-weight: 700; padding: 5px 10px; display: inline-flex; align-items: center; gap: 4px; background: #E3F0F9; color: #003B64; border: 1px solid #B3D4EC; text-decoration: none;" title="Download / Lihat Berkas PTP">
        ...
        @foreach($applications as $app)
        <form id="delete-form-{{ $app->id }}" action="{{ route('berusaha.destroy', $app->id) }}" method="POST" style="display:none;">
            @csrf
            @method('DELETE')
        </form>
        @endforeach
