        ...
        <div class="container">
                    ...
                    <div style="display: flex; align-items: center; gap: 14px;">
                        ...
                        <div>
                            <div style="font-size: 14.5px; font-weight: 800; color: #065F46; margin-bottom: 2px;">
                                Dokumen Pertek Pertanahan PSN (BPN) Telah Terbit!
                            </div>
            ...
            @if(false)
                <div class="verify-card" style="border: 1px solid #B2F5EA; background: #F0FFF4; margin-bottom: 24px; padding: 16px; border-radius: 8px;">
                    ...
                    <form action="{{ route('application.forward', ['type' => 'psn', 'id' => $application->id]) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin memajukan status permohonan ini?');">
                        @csrf
                        ...
                        <div id="Kantor Pertanahan-panel-1" class="Kantor Pertanahan-panel-step" style="display: {{ in_array($application->bpn_berkas_status, ['menunggu', 'diterima', 'tidak_sesuai', 'ditolak']) ? 'block' : 'none' }};">
                            ...
                            <fieldset {{ $isStep1Active ? '' : 'disabled' }}>
                                <form action="{{ route('psn.verify', $application->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <script>
                                         function toggleBpnSpsFieldPsn(action) {
                                    ...
                                    <div class="form-group-v" style="margin-bottom: 20px;">
                                        ...
                                        <div style="display: flex; gap: 20px;">
                                            <label style="display:flex;align-items:center;gap:6px;font-size:13.5px;font-weight:600;cursor:pointer;">
                                                <input type="radio" name="action" value="approve" required {{ $application->bpn_berkas_status === 'diterima' ? 'checked' : ($application->bpn_berkas_status === 'ditolak' || $application->bpn_berkas_status === 'tidak_sesuai' ? '' : 'checked') }} style="width:16px;height:16px;accent-color:var(--clr-blue);" onchange="toggleBpnSpsFieldPsn('approve')"> Disetujui / Lengkap
                                            </label>
                                            <label style="display:flex;align-items:center;gap:6px;font-size:13.5px;font-weight:600;color:#E53E3E;cursor:pointer;">
                                                <input type="radio" name="action" value="reject" required {{ $application->bpn_berkas_status === 'ditolak' || $application->bpn_berkas_status === 'tidak_sesuai' ? 'checked' : '' }} style="width:16px;height:16px;accent-color:var(--clr-blue);" onchange="toggleBpnSpsFieldPsn('reject')"> Tidak Lengkap
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <div id="sps-upload-container" style="display: {{ in_array($application->bpn_berkas_status, ['diterima', 'menunggu']) && $application->bpn_berkas_status !== 'tidak_sesuai' && $application->bpn_berkas_status !== 'ditolak' ? 'block' : 'none' }}; margin-bottom: 20px;">
                                        ...
                                        <input type="file" name="sps_document" id="sps_document" class="form-control-v" accept=".pdf,.jpg,.jpeg,.png" {{ !$application->bpn_sps_document && in_array($application->bpn_berkas_status, ['diterima', 'menunggu']) && $application->bpn_berkas_status !== 'tidak_sesuai' && $application->bpn_berkas_status !== 'ditolak' ? 'required' : '' }}>
                            ...
                            @if($application->bpn_pertek_document)
                                <a href="{{ route('file.view', ['path' => $application->bpn_pertek_document]) }}" target="_blank" style="display: flex; align-items: center; justify-content: space-between; background: linear-gradient(135deg, #10B981 0%, #059669 100%); color: #ffffff; padding: 12px 16px; border-radius: 8px; text-decoration: none; font-size: 13px; font-weight: 800; margin-top: 14px; box-shadow: 0 4px 12px rgba(5, 150, 105, 0.25); transition: all 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='none'">
                                    <span style="display: flex; align-items: center; gap: 8px;">
                                        📄 Dokumen Pertek Pertanahan PSN (BPN)
                                    </span>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (approvalUrl && !localStorage.getItem('downloaded_doc_approval_' + appId)) {
                ...
                activeTitle = 'Dokumen PKKPR PSN Telah Terbit!';
                activeDesc = 'Selamat! Dokumen resmi PKKPR PSN milik Anda telah diterbitkan secara lengkap.';
            } else if (pertekUrl && !localStorage.getItem('downloaded_doc_pertek_' + appId)) {
                activeDocKey = 'pertek_' + appId;
                activeUrl = pertekUrl;
                activeTitle = 'Dokumen Pertek Pertanahan PSN Telah Terbit!';
