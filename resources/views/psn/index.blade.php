...
@section('title', 'Proyek Strategis Nasional (PSN) — PATEN PAK MIKO')
...
<div style="background: #ffffff; border: 1px solid #E2E8F0; border-radius: 6px; padding: 18px 24px; margin-bottom: 20px; box-shadow: 0 2px 6px rgba(0,38,66,0.02); display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-wrap: wrap;">
    <div>
        <div style="font-size: 12px; color: #64748B; margin-bottom: 4px; display: flex; align-items: center; gap: 6px;">
            ...
            <span>›</span>
            <span style="color: #64748B;">Proyek Strategis Nasional (PSN)</span>
        </div>
        ...
        <p style="font-size: 12.5px; color: #64748B; margin: 4px 0 0;">Permohonan berbasis mandat Proyek Strategis Nasional (PSN) pemerintah.</p>
    </div>
    @if(Auth::user()->isPelakuUsaha())
        <a href="{{ route('ptp.create', ['layanan' => 'psn', 'new' => 1]) }}" class="btn btn-primary" style="border-radius: 4px; padding: 8px 16px; font-weight: 700; font-size: 13px;">
...
<div class="panel" style="border-radius: 6px; border: 1px solid #E2E8F0; box-shadow: 0 2px 6px rgba(0,38,66,0.02); overflow: hidden; background: #ffffff;">
    @if($applications->isEmpty())
        <div class="empty-state" style="padding: 40px 20px; text-align: center;">
            ...
            @if(Auth::user()->isPelakuUsaha())
                <a href="{{ route('ptp.create', ['layanan' => 'psn', 'new' => 1]) }}" class="btn btn-primary" style="border-radius: 4px;">Ajukan Sekarang</a>
            @endif
        </div>
    @else
        <!-- Filter Controls -->
        ...
        @if(Auth::user()->isDpn())
        <form id="bulkForm" action="{{ route('psn.bulk-destroy') }}" method="POST">
            @csrf
                        ...
                        <tr style="border-bottom: 1px solid #F1F5F9;" data-status-category="{{ $app->status_category }}">
                            ...
                            <td style="padding: 12px 14px; white-space: nowrap; text-align: center;">
                                <div style="display:inline-flex; gap:6px; align-items:center;">
                                    <a href="{{ route('psn.show', $app->id) }}" class="btn btn-sm btn-secondary" style="border-radius: 4px; font-size: 12px; font-weight: 700; padding: 5px 10px; display: inline-flex; align-items: center; gap: 4px;">
                                        Detail
                                    ...
                                    <select onchange="if(this.value){ if(this.options[this.selectedIndex].getAttribute('data-target')==='_blank'){ window.open(this.value, '_blank'); } else { window.location.href = this.value; } this.selectedIndex = 0; }" style="border-radius: 4px; font-size: 12px; font-weight: 700; padding: 5px 8px; background: #E3F0F9; color: #003B64; border: 1px solid #B3D4EC; cursor: pointer; outline: none; height: 30px;">
                                        ...
                                        <option value="{{ route('psn.ptp_pdf', $app->id) }}" data-target="_blank">📄 Preview PDF</option>
                                        <option value="{{ route('psn.ptp_pdf', $app->id) }}?action=download" data-target="_self">📥 Download DOCX</option>
                                    </select>
                                    @endif
                                    @if(Auth::user()->isDpn())
        ...
        @foreach($applications as $app)
        <form id="delete-form-{{ $app->id }}" action="{{ route('psn.destroy', $app->id) }}" method="POST" style="display:none;">
            @csrf
            @method('DELETE')
        </form>
        @endforeach
