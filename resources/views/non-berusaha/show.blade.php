        ...
        <div class="container">
            ...
            @if($application->status === 'disetujui')
                ...
                @elseif($application->approval_document)
                    <a href="{{ route('file.view', ['path' => $application->approval_document]) }}" target="_blank" class="btn-download-cert" style="margin-bottom: 20px;">
                        <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                        Unduh Dokumen PKKPR Non Berusaha (PDF)
                    </a>
                @endif
            @endif
            ...
            @if(Auth::user()->isDpn() && !($application->status === 'menunggu_bpn' && ($application->bpn_berkas_status === 'menunggu' || !$application->bpn_berkas_status)))
                <div class="verify-card" style="border: 1px solid #FBD38D; background: #FFFAF0; margin-bottom: 24px; padding: 16px; border-radius: 8px;">
                    ...
                    <form action="{{ route('application.rollback', ['type' => 'ppkpr_non_berusaha', 'id' => $application->id]) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mengembalikan status permohonan ini ke tahap sebelumnya?');">
                        @csrf
                        ...
                        <div id="Kantor Pertanahan-panel-1" class="Kantor Pertanahan-panel-step" style="display: {{ in_array($application->bpn_berkas_status, ['menunggu', 'diterima', 'tidak_sesuai', 'ditolak']) ? 'block' : 'none' }};">
                            ...
                            <fieldset {{ $isStep1Active ? '' : 'disabled' }}>
                                <form action="{{ route('non-berusaha.verify', $application->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <script>
                                        function toggleBpnSpsFieldNonBerusaha(action) {
                                    ...
                                    <div class="form-group-v" style="margin-bottom: 20px;">
                                        ...
                                        <div style="display: flex; gap: 20px;">
                                            <label style="display:flex;align-items:center;gap:6px;font-size:13.5px;font-weight:600;cursor:pointer;">
                                                <input type="radio" name="action" value="approve" required {{ $application->bpn_berkas_status === 'diterima' ? 'checked' : ($application->bpn_berkas_status === 'ditolak' || $application->bpn_berkas_status === 'tidak_sesuai' ? '' : 'checked') }} style="width:16px;height:16px;accent-color:var(--clr-blue);" onchange="toggleBpnSpsFieldNonBerusaha('approve')"> Disetujui / Lengkap
                                            </label>
                                            <label style="display:flex;align-items:center;gap:6px;font-size:13.5px;font-weight:600;color:#E53E3E;cursor:pointer;">
                                                <input type="radio" name="action" value="reject" required {{ $application->bpn_berkas_status === 'ditolak' || $application->bpn_berkas_status === 'tidak_sesuai' ? 'checked' : '' }} style="width:16px;height:16px;accent-color:var(--clr-blue);" onchange="toggleBpnSpsFieldNonBerusaha('reject')"> Tidak Lengkap
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <div id="satuPintuFieldsWrapper">
                                        ...
                                        <div class="form-group-v" style="margin-bottom:12px;">
                                            @if($application->approval_document)
                                                <label class="form-label" style="font-weight:700;color:#744210;">Dokumen PKKPR Non Berusaha</label>
                                            ...
                                            @else
                                                <label for="approval_document" class="form-label" style="font-weight:700;color:#744210;">Unggah Dokumen PKKPR Non Berusaha (opsional)</label>
                ...
                <div>
                    <div class="card">
                        ...
                        <ul class="detail-list">
                            ...
                            @if($application->bpn_sps_document)
                                <li class="detail-item" style="padding: 10px 0;">
                                    <span class="detail-label" style="font-weight: 800; color: #0369A1;">Dokumen Tagihan SPS (BPN)</span>
                                    <span class="detail-val">
                                        <a href="{{ route('file.view', ['path' => $application->bpn_sps_document]) }}" target="_blank" style="display: inline-flex; align-items: center; gap: 8px; background: linear-gradient(135deg, #0284C7 0%, #0369A1 100%); color: #ffffff; padding: 9px 16px; border-radius: 8px; text-decoration: none; font-size: 12.5px; font-weight: 800; box-shadow: 0 3px 10px rgba(2, 132, 199, 0.25); transition: all 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='none'">
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: center;">
                                    ...
                                    <div style="position: relative;">
                                        ...
                                        <div id="ptpAdminDropdown" style="display: none; position: absolute; top: 100%; right: 0; margin-top: 8px; background: white; border: 1px solid #E2E8F0; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); width: max-content; overflow: hidden; z-index: 10; text-align: left;">
                                            <a href="{{ route('non-berusaha.ptp_pdf', $application->id) }}" target="_blank" style="display: flex; align-items: center; gap: 8px; padding: 10px 16px; text-decoration: none; color: #2D3748; font-size: 13px; font-weight: 600; border-bottom: 1px solid #E2E8F0;" onmouseover="this.style.background='#F7FAFC'" onmouseout="this.style.background='white'">
                                            ...
                                            <a href="{{ route('non-berusaha.ptp_pdf', $application->id) }}?action=download" style="display: flex; align-items: center; gap: 8px; padding: 10px 16px; text-decoration: none; color: #2D3748; font-size: 13px; font-weight: 600;" onmouseover="this.style.background='#F7FAFC'" onmouseout="this.style.background='white'">
                ...
                <div>
                    ...
                    <div class="card" style="position: sticky; top: 88px; max-height: calc(100vh - 100px); overflow-y: auto;">
                        ...
                        <div class="timeline">
                            ...
                            @php
                                $step5Status = '';
                                if ($hasPassedPertek || !empty($application->bpn_pertek_document)) {
                            ...
                            <div class="timeline-step {{ $step7Status }}" onclick="showBpnPanel('satu-pintu')" style="cursor:pointer;">
                                ...
                                <div class="timeline-content">
                                    ...
                                    <div class="timeline-desc">
                                        DPMPTSP menerbitkan dokumen PKKPR Non Berusaha resmi.
                                    </div>
                                    @if($application->satu_pintu_no_pkkpr)
                                        <div class="timeline-notes" style="border-left-color: var(--clr-green); background: #F4FBF7; color: #137333;">
                                            <strong>No. PKKPR Non Berusaha:</strong> {{ $application->satu_pintu_no_pkkpr }}
                                        </div>
                                    @endif
