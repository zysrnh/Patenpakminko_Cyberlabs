            ...
            <nav class="site-nav" aria-label="Main navigation">
                ...
                @php
                    ...
                    if (\Illuminate\Support\Facades\Storage::disk('local')->exists('whatsapp_settings.json')) {
                        $pubSettings = json_decode(\Illuminate\Support\Facades\Storage::disk('local')->get('whatsapp_settings.json'), true) ?? [];
                    }
                    $pubPhone = $pubSettings['contact_phone'] ?? ($pubSettings['cp_admin'] ?? '6281322712133');
                    $cleanPubPhone = preg_replace('/[^0-9]/', '', $pubPhone);
                    if (str_starts_with($cleanPubPhone, '0')) {
                        $cleanPubPhone = '62' . substr($cleanPubPhone, 1);
                    }
                    $pubWaUrl = 'https://wa.me/' . $cleanPubPhone;
                @endphp
                <a href="{{ $pubWaUrl }}" target="_blank" class="btn-nav">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
